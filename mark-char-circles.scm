...
; work with.
(define (mark-letter-circles radius image letters)
  (let ((vec (car (drop (gimp-path-get-points image
    ...
    (make-circles (vector->list vec) radius image letters)))
...
; mark-letter-circles function for detais.  radius is the circle radius and
; image is the image to work with.
(define (make-circles lis radius image letters)
  ...
  (let ((i 1))
    (while (pair? lis)
           ...
           (let ((x (car lis))
             ...
             (make-circle x y radius (if (letters) (list-ref (list "a" "b" "c" "d" "e" "f" "g" "h" "i" "j" "k" "l" "m" "n" "o" "p" "q" "r" "s" "t" "u" "v" "w") (- i 1))) image))
...
(script-fu-register "mark-letter-circles"
                    "Mark Letter Circles"
                    "Produce \"circles\" to be used as letter marks"
                    ...
                    SF-IMAGE    "Image"             0
                    SF-TOGGLE   "Use letters"    TRUE
                    )

; Register the script in the menu
(script-fu-menu-register "mark-letter-circles" "<Image>/Script-Fu")
